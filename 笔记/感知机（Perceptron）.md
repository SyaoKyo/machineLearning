...
感知机定义：假设输入空间（特征空间）是 ![](http://latex.codecogs.com/gif.latex?\\\mathcal{X}\in R^n)，输出空间是![](http://latex.codecogs.com/gif.latex?\\\mathcal{Y}=\lbrace+1,-1\rbrace),输入![](http://latex.codecogs.com/gif.latex?\\\in\mathcal{X})表示实例的特征向量，对应于输入空间（特征空间）的点；输出![](http://latex.codecogs.com/gif.latex?\\y\in\mathcal{Y})表示实例的类别，由输入空间到输出空间的模型称为感知机。如下函数：
![](http://latex.codecogs.com/gif.latex?\\
f(x)=sign(w \cdot x + b)
)
其中，![](http://latex.codecogs.com/gif.latex?\\w)和![](http://latex.codecogs.com/gif.latex?\\b)为感知机模型参数，![](http://latex.codecogs.com/gif.latex?\\w\in R^n)叫作权值（weight）或权值向量（weight vector），![](http://latex.codecogs.com/gif.latex?\\b\in R)叫作偏置（bias），![](http://latex.codecogs.com/gif.latex?\\w\cdot x)表示![](http://latex.codecogs.com/gif.latex?\\w)和![](http://latex.codecogs.com/gif.latex?\\x)的内积。sign是符号函数，即
![](http://latex.codecogs.com/gif.latex?\\
sign(x)=\left\{
\begin{array}{rcl}
  +1, & x\geq 0\\
...
其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n,y_i\in\mathcal{Y}=\{+1,-1\},i= 1,2,\cdots,N),求得感知机模型，即求得模型参数![](http://latex.codecogs.com/gif.latex?\\w,b)。感知机预测，通过学习得到的感知机模型，对于新的输入实例给出其对应的输出类别。
...
其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n,y_i\in\mathcal{Y}=\{+1,-1\},i= 1,2,\cdots,N),如果存在某个超平面S
![](http://latex.codecogs.com/gif.latex?\\
w \cdot x + b = 0
)
能够将数据集的正实例点和负实例点完全正确的划分到超平面的两侧，即对所有![](http://latex.codecogs.com/gif.latex?\\y_i=+1)的实例![](http://latex.codecogs.com/gif.latex?\\i)，有![](http://latex.codecogs.com/gif.latex?\\w \cdot x + b > 0)，对所有![](http://latex.codecogs.com/gif.latex?\\y_i=-1)的实例![](http://latex.codecogs.com/gif.latex?\\i)，有![](http://latex.codecogs.com/gif.latex?\\w \cdot x + b < 0)，则称数据集T为线性可分数据集（linearly separable data set）；否则，称数据集T线性不可分。
...
成立。因为当 ![](http://latex.codecogs.com/gif.latex?\\w \cdot x + b > 0) 时，![](http://latex.codecogs.com/gif.latex?\\y_i=-1) ，当![](http://latex.codecogs.com/gif.latex?\\w \cdot x + b < 0) 时，![](http://latex.codecogs.com/gif.latex?\\y_i=+1)。所以误分类点 ![[公式]](https://www.zhihu.com/equation?tex=x_%7Bi%7D) 到分离超平面的距离:
![](http://latex.codecogs.com/gif.latex?\\
...
其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n,y_i\in\mathcal{Y}=\{+1,-1\},i= 1,2,\cdots,N)。感知机![](http://latex.codecogs.com/gif.latex?\\sign(w \cdot x + b))的损失函数定义为
![](http://latex.codecogs.com/gif.latex?\\
...
式中![](http://latex.codecogs.com/gif.latex?\\\eta(0 < \eta \leq 1))是步长，在统计学习中又称为学习率（learning rate）。这样，通过迭代可以期待损失函数![](http://latex.codecogs.com/gif.latex?\\L(w,b))不断减小，直到0.综上所述，得到如下算法：

感知机学习算法（原始形式）

> 输入：训练数据集![](http://latex.codecogs.com/gif.latex?\\T=\{(x_1,y_1),(x_2,y_2),\cdots,(x_n,y_n)\})其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n)，![](http://latex.codecogs.com/gif.latex?\\y_i\in\mathcal{Y}=\{-1,1\})，![](http://latex.codecogs.com/gif.latex?\\i= 1,2,\cdots,N)；学习率![](http://latex.codecogs.com/gif.latex?\\\eta(0 < \eta \leq 1))；
>
> 输出：![](http://latex.codecogs.com/gif.latex?\\w,b)；感知机模型![](http://latex.codecogs.com/gif.latex?\\f(x)=sign(w \cdot x + b))。
>
> （1）选取初值![](http://latex.codecogs.com/gif.latex?\\w_0,b_0);
>
> （2）在训练集中选取数据![](http://latex.codecogs.com/gif.latex?\\(x_i,y_i))；
>
> （3）如果![](http://latex.codecogs.com/gif.latex?\\y_i(w \cdot x_i + b) \leq 0),
> ![](http://latex.codecogs.com/gif.latex?\\
> w \leftarrow w + \eta y_ix_i \\
> b \leftarrow b + \eta y_i
> )
...
**定理.Novikoff** 设训练数据集![](http://latex.codecogs.com/gif.latex?\\T=\{(x_1,y_1),(x_2,y_2),\cdots,(x_n,y_n)\})是线性可分的， 其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n)，![](http://latex.codecogs.com/gif.latex?\\y_i\in\mathcal{Y}=\{-1,1\})，![](http://latex.codecogs.com/gif.latex?\\i= 1,2,\cdots,N)，则：

（1）存在满足条件  ![](http://latex.codecogs.com/gif.latex?\\\|\hat{w}_{opt}\|=1)的超平面  ![](http://latex.codecogs.com/gif.latex?\\\hat{w}_{opt}\cdot \hat{x}=w_{opt}x + b_{opt} = 0)将训练数据集完整正确分开；并存在![](http://latex.codecogs.com/gif.latex?\\\gamma > 0)，对所有 ![](http://latex.codecogs.com/gif.latex?\\i= 1,2,\cdots,N)
![](http://latex.codecogs.com/gif.latex?\\
y_i(\hat{w}_{opt}\cdot \hat{x}_i)=y_i(w_{opt}x_i + b_{opt})\geq\gamma
)
（2）令 ![](http://latex.codecogs.com/gif.latex?\\R = \max \limits_{1\leq i \leq N}\|\hat{x}_i\|)，则感知机算法在训练集上的误分类次数 ![](http://latex.codecogs.com/gif.latex?\\k)满足不等式：
![](http://latex.codecogs.com/gif.latex?\\
...
> 输入：训练数据集![](http://latex.codecogs.com/gif.latex?\\T=\{(x_1,y_1),(x_2,y_2),\cdots,(x_n,y_n)\})其中，![](http://latex.codecogs.com/gif.latex?\\x_i\in\mathcal{X}=R^n)，![](http://latex.codecogs.com/gif.latex?\\y_i\in\mathcal{Y}=\{-1,1\})，![](http://latex.codecogs.com/gif.latex?\\i= 1,2,\cdots,N)；学习率![](http://latex.codecogs.com/gif.latex?\\\eta(0 < \eta \leq 1))；
>
> 输出：![](http://latex.codecogs.com/gif.latex?\\a,b)；感知机模型![](http://latex.codecogs.com/gif.latex?\\f(x)=sign(\sum\limits^{N}_{i=1}a_jy_jx_j\cdot x + b))，其中![](http://latex.codecogs.com/gif.latex?\\a=(a_1,a_2,\cdots,a_N)^T)。
>
> （1）![](http://latex.codecogs.com/gif.latex?\\a\leftarrow0,b\leftarrow0);
>
> （2）在训练集中选取数据![](http://latex.codecogs.com/gif.latex?\\(x_i,y_i))；
>
> （3）如果![](http://latex.codecogs.com/gif.latex?\\y_i(\sum\limits^{N}_{i=1}a_jy_jx_j\cdot x + b) \leq 0),
> ![](http://latex.codecogs.com/gif.latex?\\
...
1. 感知机是根据输入实例的特征向量![](http://latex.codecogs.com/gif.latex?\\x)对其进行二类分类的线性分类模型：
   ![](http://latex.codecogs.com/gif.latex?\\
   f(x)=sign(w \cdot x + b))
   感知机模型对应于输入空间（特征空间）中的分离超平面![](http://latex.codecogs.com/gif.latex?\\w \cdot x + b=0)
